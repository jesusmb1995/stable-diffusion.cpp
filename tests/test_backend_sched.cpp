...
TEST_F(PSNRIntegrationTest, SchedOutputMatchesSingleBackend) {
    ...
    // With two CPU backends, results must be bitwise identical (infinite PSNR).
    // Use >200 dB threshold to account for floating-point representation.
    EXPECT_GT(psnr, 200.0)
...
// Creates a CPU backend with its own private device struct that rejects
// certain ops in supports_op(). Since all CPU backends share a single
// global device, we allocate a separate ggml_backend_device and point
// the backend at it so patching the vtable doesn't affect other backends.
struct RestrictedBackend {
    ...
    static bool restricted_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor* op) {
        auto* self = static_cast<RestrictedBackend*>(dev->context);
...
static RestrictedBackend* create_restricted_cpu(std::set<enum ggml_op> rejected_ops) {
    ...
    // Clone the real device so we have our own copy of the vtable
...
// PSNR threshold is relaxed (>30 dB) because GPU floating-point
// rounding may differ slightly from CPU.
